...
{
  let mut buf = Vec::new();
  packet.id().write_varint(&mut buf).ok()?;
...
pub fn compression_encoder(data: &[u8], compression_threshold: u32) -> Option<Vec<u8>> {
  ...
  if n < compression_threshold as usize {
    ...
    (data.len() as u32).write_varint(&mut len_prepended_compressed_data).ok()?;
...
fn frame_prepender(mut data: Vec<u8>) -> Option<Vec<u8>> {
  ...
  (data.len() as u32).write_varint(&mut buf).ok()?;
  buf.append(&mut data);
